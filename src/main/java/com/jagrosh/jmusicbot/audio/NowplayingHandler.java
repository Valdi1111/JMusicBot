...
public class NowplayingHandler {
    ...
    public void updateTopic(long guildId, AudioHandler handler, boolean wait) {
        ...
        if (guild == null)
            return;
        Settings settings = bot.getDataManager().getSettings(guildId);
        Optional<TextChannel> opt = settings.getTextChannel();
        if (opt.isPresent() && guild.getSelfMember().hasPermission(opt.get(), Permission.MANAGE_CHANNEL)) {
            TextChannel tchan = opt.get();
            String otherText;
