...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler {
    ...
    public boolean playFromDefault() {
        Settings settings = manager.getBot().getDataManager().getSettings(guildId);
        Optional<Playlist> playlist = settings.getDefaultPlaylist();
        ...
        playlist.get().loadNextTrack(guild, manager, audioPlayer::playTrack, () -> {
            if (playlist.get().getTracks().isEmpty() && !manager.getBot().getConfig().isStayInChannel())
    ...
    public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
        ...
        if (endReason == AudioTrackEndReason.FINISHED && manager.getBot().getDataManager().getSettings(guildId).isRepeatMode()) {
    ...
    public Message getNowPlaying(JDA jda) {
        if (isMusicPlaying(jda)) {
            ...
            mb.append(FormatUtil.filter(manager.getBot().getConfig().getEmojiSuccess() + " **Now Playing in " + guild.getSelfMember().getVoiceState().getChannel().getName() + "...**"));
            ...
            Optional<Track> t = TrackUtils.getTrack(manager.getBot().getDataManager().getDatabase(guild), track);
            if (t.isPresent()) {
                title = t.get().getTitle();
                author = t.get().getArtist();
                url = t.get().getFile();
                thumbnail = t.get().getThumbnail();
            }
            ...
            if (manager.getBot().getConfig().isNowPlayingImages()) {
                if (!thumbnail.isEmpty()) {
                    eb.setThumbnail(manager.getBot().getMain().getWebServer().getAccessAddress() + "/data/cache/" + guildId + "/" + thumbnail);
    ...
    public Message getNoMusicPlaying(JDA jda) {
        ...
        return new MessageBuilder()
                .setContent(FormatUtil.filter(manager.getBot().getConfig().getEmojiSuccess() + " **Now Playing...**"))
